...
def assemble(code):
    ...
    try:
        encoded, count = ks.asm(code)
    except KsError as e:
        print(f"ERROR: {e}")
    bytestr = bytes(encoded)
    return bytestr


def disassemble(bytestr):
    code = ""
    cs = Cs(CS_ARCH_X86, CS_MODE_64)
    for i in cs.disasm(bytestr, 0x1000):
        # line = f"0x{i.address:x}:\t{i.mnemonic}\t{i.op_str}"
        line = f"{i.mnemonic:<10}{i.op_str}"
        code += f"{line}\n"
    return code
...
def inject(binary, sc):
    ...
    out.build()
    file_patched = "out" + os.path.basename(binary)
...
def main():
    ...
    # WORKS
    # bytestr = (  # 433
    #     "\x48\x83\xEC\x28\x48\x83\xE4\xF0\x48\x8D\x15\x66\x00\x00\x00"  # 15
    ...
    # )
    # bytestr = bytestr.encode("latin-1")
